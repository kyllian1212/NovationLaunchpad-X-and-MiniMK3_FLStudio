...
def metronome():
    ...
    if currentBeat in beat1_steps and transport.isPlaying():
        if transport.getSongLength(0) == 0 and transport.getLoopMode() == 1:
            lp.lightPad(pc.METRONOME_DISPLAYPAD, lp.color["off"], lp.state["static"])
        else:    
            lp.lightPad(pc.METRONOME_DISPLAYPAD, lp.color["light_orange"], lp.state["static"])
    elif currentBeat in beat234_steps and transport.isPlaying():
        lp.lightPad(pc.METRONOME_DISPLAYPAD, lp.color["darker_orange"], lp.state["static"])
    else:
        lp.lightPad(pc.METRONOME_DISPLAYPAD, lp.color["off"], lp.state["static"])